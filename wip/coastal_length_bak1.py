...
class Coastal:
    ...
    row: dict[int, list[tuple[int, int]]] = field(default_factory=dict)
    col: dict[int, list[tuple[int, int]]] = field(default_factory=dict)
    count: int = 0
    ...
    def costal_1_check(self, y: int, x: int) -> None:
        ...
        if check(y - 1, x):
            self.col.append((y,x))
        if check(y + 1, x):
            self.col.append((y+1, x))
        if check(y, x - 1):
            self.row.append((y, x))
        if check(y, x + 1):
            self.row.append((y, x+1))
        return
